...
func CreateCategory(c *gin.Context) {
	...
	orderNumberInHomePage, orderNumber, err := models.ValidateCategory("", category.ParentCategoryID.String, category.Image, "create", category.OrderNumber, 0, category.OrderNumberInHomePage, 0, category.IsHomeCategory)
	if err != nil {
		helpers.HandleError(c, 400, err.Error())
		return
	}
	...
	if orderNumberInHomePage == 0 {
		orderNumberInHomePage = category.OrderNumberInHomePage
	}
	...
	db.QueryRow(context.Background(), "INSERT INTO categories (parent_category_id,image,is_home_category,order_number,is_visible,order_number_in_home_page) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id", parent_category_id, category.Image, category.IsHomeCategory, orderNumber, category.IsVisible, orderNumberInHomePage).Scan(&categoryID)
...
func UpdateCategoryByID(c *gin.Context) {
	...
	// var fileName string
	var parent_category_id interface{}
	orderNumberInHomePage, orderNumber, err := models.ValidateCategory(ID, category.ParentCategoryID.String, "", "update", category.OrderNumber, category.OldOrderNumber, category.OrderNumberInHomePage, category.OldOrderNumberInHomePage, category.IsHomeCategory)
	if err != nil {
		helpers.HandleError(c, 400, err.Error())
		return
	}
...
func GetAllCategoryForHeader(langID, search, searchStr string, limit, page uint, isVisible bool) ([]ResultCategory, uint, error) {
	...
	for rows.Next() {
		var result ResultCategory
		if err := rows.Scan(&result.ID, &result.Image, &result.Name, &result.OrderNumber, &result.OrderNumberInHomePage, &result.IsVisible); err != nil {
			return []ResultCategory{}, 0, err
		}
		...
		for rowss.Next() {
			var resul ResultCategor
			if err := rowss.Scan(&resul.ID, &resul.Name, &resul.OrderNumber, &resul.OrderNumberInHomePage, &resul.IsVisible); err != nil {
				return []ResultCategory{}, 0, err
			}
			...
			for rowsss.Next() {
				var resu ResultCatego
				if err := rowsss.Scan(&resu.ID, &resu.Name, &resu.OrderNumber, &resu.OrderNumberInHomePage, &resu.IsVisible); err != nil {
					return []ResultCategory{}, 0, err
				}
