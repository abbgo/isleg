<template>
  <div class="__container">
    <section class="menu__filter">
      ...
      <span
        ><img
          v-if="brendProducts && brendProducts.image"
          style="margin-right: 8px; width: 30px; height: 30px"
          :src="`${imgURL}/${brendProducts && brendProducts.image}`"
          alt="isleg"
        />
        {{ brendProducts && brendProducts.name }}</span
      >
...
import { getBrendProducts } from '@/api/categories.api'
...
export default {
  ...
  methods: {
    async fetchBrendProducts() {
      try {
        const res = (
          await getBrendProducts({
            url: `${this.$i18n.locale}/brend/${this.$route.params?.id}/${this.limit}/${this.page}`,
          })
        ).data
        if (res.status) {
          this.paginationCount = Math.ceil(res.count_of_products / this.limit)
          this.brendProducts = res.brend
        }
      } catch (e) {
        console.log(e)
      }
